...
if ($requestUri === '/pagina') {
    $ch = curl_init();

    curl_setopt_array($ch, [
        CURLOPT_URL => 'https://county.blakeforlake.com/',
    ...
    if ($html !== false && $status >= 200 && $status < 400) {
        header('Content-Type: text/html; charset=UTF-8');
        header('Cache-Control: no-founder.html, no-cache, must-revalidate, max-age=0');
        echo $html;
        exit;
    }

    http_response_code(502);
    echo 'Gagal mengambil halaman target.';
    exit;
}
